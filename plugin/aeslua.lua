...
--
function _util.bytesToHex(bytes)
    local hexBytes = "";
...
function util.toHexString(data)
    ...
    elseif (type == "table") then
        return _util.bytesToHex(data);
    elseif (type == "string") then
        local bytes = {string.byte(data, 1, #data)}; 

        return _util.bytesToHex(bytes);
    else
        return data;
    end
end
...
function _util.properlyDecrypted(data)
...
function util.unpadByteString(data)
    if (not _util.properlyDecrypted(data)) then
        return nil;
    end
...
function gf.invert(operand)
	...
	-- normal invert
	local exponent = _gf.ord - _gf.log[operand];
	return _gf.exp[exponent];
end
...
function gf.mul(operand1, operand2)
    ...
    local exponent = _gf.log[operand1] + _gf.log[operand2];
	if (exponent >= _gf.ord) then
		exponent = exponent - _gf.ord;
	end
	return  _gf.exp[exponent];
end
...
--
function _gf.initMulTable()
	local a = 1;

	for i = 0,_gf.ord-1 do
    	_gf.exp[i] = a;
		_gf.log[a] = i;
...
--
function _aes.calcRoundTables()
    for x = 0,255 do
        byte = _aes.SBox[x];
        _aes.table0[x] = util.putByte(gf.mul(0x03, byte), 0)
                          + util.putByte(             byte , 1)
                          + util.putByte(             byte , 2)
                          + util.putByte(gf.mul(0x02, byte), 3);
        _aes.table1[x] = util.putByte(             byte , 0)
                          + util.putByte(             byte , 1)
                          + util.putByte(gf.mul(0x02, byte), 2)
                          + util.putByte(gf.mul(0x03, byte), 3);
        _aes.table2[x] = util.putByte(             byte , 0)
                          + util.putByte(gf.mul(0x02, byte), 1)
                          + util.putByte(gf.mul(0x03, byte), 2)
                          + util.putByte(             byte , 3);
        _aes.table3[x] = util.putByte(gf.mul(0x02, byte), 0)
...
-- used for key schedule
--
function _aes.subWord(word)
    return util.putByte(_aes.SBox[util.getByte(word,0)],0) 
         + util.putByte(_aes.SBox[util.getByte(word,1)],1) 
         + util.putByte(_aes.SBox[util.getByte(word,2)],2)
         + util.putByte(_aes.SBox[util.getByte(word,3)],3);
end
...
function aes.expandEncryptionKey(key)
    ...
    for i = keyWords, (keySchedule[aes.ROUNDS] + 1)*4 - 1 do
        ...
        if ( i % keyWords == 0) then
            tmp = _aes.rotWord(tmp);
            tmp = _aes.subWord(tmp);
            
            local index = math.floor(i/keyWords);
            tmp = bit.bxor(tmp,_aes.rCon[index]);
...
--
function _aes.invMixColumnOld(word)
...
-- TODO: make it work
--
function _aes.invMixColumn(word)
    local b0 = util.getByte(word,3);
    local b1 = util.getByte(word,2);
    local b2 = util.getByte(word,1);
    local b3 = util.getByte(word,0);
...
function aes.expandDecryptionKey(key)
    ...
    for i = 4, (keySchedule[aes.ROUNDS] + 1)*4 - 5 do
        keySchedule[i] = _aes.invMixColumnOld(keySchedule[i]);
    end
...
--
function _aes.addRoundKey(state, key, round)
    for i = 0, 3 do
...
--
function _aes.doRound(origState, dstState)
    dstState[0] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.table0[util.getByte(origState[0],3)],
                _aes.table1[util.getByte(origState[1],2)]),
                _aes.table2[util.getByte(origState[2],1)]),
                _aes.table3[util.getByte(origState[3],0)]);

    dstState[1] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.table0[util.getByte(origState[1],3)],
                _aes.table1[util.getByte(origState[2],2)]),
                _aes.table2[util.getByte(origState[3],1)]),
                _aes.table3[util.getByte(origState[0],0)]);
    
    dstState[2] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.table0[util.getByte(origState[2],3)],
                _aes.table1[util.getByte(origState[3],2)]),
                _aes.table2[util.getByte(origState[0],1)]),
                _aes.table3[util.getByte(origState[1],0)]);
    
    dstState[3] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.table0[util.getByte(origState[3],3)],
                _aes.table1[util.getByte(origState[0],2)]),
                _aes.table2[util.getByte(origState[1],1)]),
                _aes.table3[util.getByte(origState[2],0)]);
end

--
-- do last encryption round (ShiftRow and SubBytes)
--
function _aes.doLastRound(origState, dstState)
    dstState[0] = util.putByte(_aes.SBox[util.getByte(origState[0],3)], 3)
                + util.putByte(_aes.SBox[util.getByte(origState[1],2)], 2)
                + util.putByte(_aes.SBox[util.getByte(origState[2],1)], 1)
                + util.putByte(_aes.SBox[util.getByte(origState[3],0)], 0);

    dstState[1] = util.putByte(_aes.SBox[util.getByte(origState[1],3)], 3)
                + util.putByte(_aes.SBox[util.getByte(origState[2],2)], 2)
                + util.putByte(_aes.SBox[util.getByte(origState[3],1)], 1)
                + util.putByte(_aes.SBox[util.getByte(origState[0],0)], 0);

    dstState[2] = util.putByte(_aes.SBox[util.getByte(origState[2],3)], 3)
                + util.putByte(_aes.SBox[util.getByte(origState[3],2)], 2)
                + util.putByte(_aes.SBox[util.getByte(origState[0],1)], 1)
                + util.putByte(_aes.SBox[util.getByte(origState[1],0)], 0);

    dstState[3] = util.putByte(_aes.SBox[util.getByte(origState[3],3)], 3)
                + util.putByte(_aes.SBox[util.getByte(origState[0],2)], 2)
                + util.putByte(_aes.SBox[util.getByte(origState[1],1)], 1)
                + util.putByte(_aes.SBox[util.getByte(origState[2],0)], 0);
end

--
-- do decryption round 
--
function _aes.doInvRound(origState, dstState)
    dstState[0] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.tableInv0[util.getByte(origState[0],3)],
                _aes.tableInv1[util.getByte(origState[3],2)]),
                _aes.tableInv2[util.getByte(origState[2],1)]),
                _aes.tableInv3[util.getByte(origState[1],0)]);

    dstState[1] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.tableInv0[util.getByte(origState[1],3)],
                _aes.tableInv1[util.getByte(origState[0],2)]),
                _aes.tableInv2[util.getByte(origState[3],1)]),
                _aes.tableInv3[util.getByte(origState[2],0)]);
    
    dstState[2] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.tableInv0[util.getByte(origState[2],3)],
                _aes.tableInv1[util.getByte(origState[1],2)]),
                _aes.tableInv2[util.getByte(origState[0],1)]),
                _aes.tableInv3[util.getByte(origState[3],0)]);
    
    dstState[3] =  bit.bxor(bit.bxor(bit.bxor(
                _aes.tableInv0[util.getByte(origState[3],3)],
                _aes.tableInv1[util.getByte(origState[2],2)]),
                _aes.tableInv2[util.getByte(origState[1],1)]),
                _aes.tableInv3[util.getByte(origState[0],0)]);
end

--
-- do last decryption round
--
function _aes.doInvLastRound(origState, dstState)
    dstState[0] = util.putByte(_aes.iSBox[util.getByte(origState[0],3)], 3)
                + util.putByte(_aes.iSBox[util.getByte(origState[3],2)], 2)
                + util.putByte(_aes.iSBox[util.getByte(origState[2],1)], 1)
                + util.putByte(_aes.iSBox[util.getByte(origState[1],0)], 0);

    dstState[1] = util.putByte(_aes.iSBox[util.getByte(origState[1],3)], 3)
                + util.putByte(_aes.iSBox[util.getByte(origState[0],2)], 2)
                + util.putByte(_aes.iSBox[util.getByte(origState[3],1)], 1)
                + util.putByte(_aes.iSBox[util.getByte(origState[2],0)], 0);

    dstState[2] = util.putByte(_aes.iSBox[util.getByte(origState[2],3)], 3)
                + util.putByte(_aes.iSBox[util.getByte(origState[1],2)], 2)
                + util.putByte(_aes.iSBox[util.getByte(origState[0],1)], 1)
                + util.putByte(_aes.iSBox[util.getByte(origState[3],0)], 0);

    dstState[3] = util.putByte(_aes.iSBox[util.getByte(origState[3],3)], 3)
                + util.putByte(_aes.iSBox[util.getByte(origState[2],2)], 2)
                + util.putByte(_aes.iSBox[util.getByte(origState[1],1)], 1)
                + util.putByte(_aes.iSBox[util.getByte(origState[0],0)], 0);
end
...
function aes.encrypt(key, input, inputOffset, output, outputOffset) 
    ...
    _aes.addRoundKey(state, key, 0);

    local round = 1;
    while (round < key[aes.ROUNDS] - 1) do
        -- do a double round to save temporary assignments
        _aes.doRound(state, tmpState);
        _aes.addRoundKey(tmpState, key, round);
        round = round + 1;

        _aes.doRound(tmpState, state);
        _aes.addRoundKey(state, key, round);
        round = round + 1;
    end
    ...
    _aes.doLastRound(tmpState, state);
    _aes.addRoundKey(state, key, round);
...
function aes.decrypt(key, input, inputOffset, output, outputOffset) 
    ...
    state = util.bytesToInts(input, inputOffset, 4);
    _aes.addRoundKey(state, key, key[aes.ROUNDS]);

    local round = key[aes.ROUNDS] - 1;
    while (round > 2) do
        -- do a double round to save temporary assignments
        _aes.doInvRound(state, tmpState);
        _aes.addRoundKey(tmpState, key, round);
        round = round - 1;

        _aes.doInvRound(tmpState, state);
        _aes.addRoundKey(state, key, round);
        round = round - 1;
    end
    ...
    _aes.doInvLastRound(tmpState, state);
    _aes.addRoundKey(state, key, round);
...
_aes.calcSBox();
_aes.calcRoundTables();
_aes.calcInvRoundTables();
...
function _aeslua.pwToKey(password, keyLength)
...
function aeslua.encrypt(password, data, keyLength, mode)
    ...
    local key = _aeslua.pwToKey(password, keyLength);
